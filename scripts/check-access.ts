...
type CallOptions = { cookie?: string; body?: unknown }

async function call(method: string, path: string, { cookie = '', body }: CallOptions = {}) {
  const res = await fetch(base + path, {
    method,
    headers: { ...(body ? json : {}), ...(cookie ? { Cookie: cookie } : {}) },
...
const setOpen = (openAccess: boolean) => call('POST', '/api/globals/event', { cookie: couple, body: { openAccess } })
...
try {
  for (const open of [true, false]) {
    ...
    for (const [who, cookie] of [
      ...
      check(
        (await call('POST', '/api/globals/event', { cookie, body: { openAccess: !open } })).status === 403,
        'cannot change the Wedding global (Open access included)',
      )
      check(
        (await call('POST', '/api/users', { cookie, body: { email: 'x@example.com', password: 'abcdefgh1!' } }))
          .status === 403,
        'cannot create a couple login',
      )
      check((await call('POST', '/api/guests', { cookie, body: { name: 'x' } })).status === 403, 'cannot create guests')
      check(
        (await call('POST', '/api/guests/quiz-tag', { cookie, body: { mode: 'all' } })).status === 403,
        'cannot tag guests for the quiz',
      )

      const guests = await call('GET', '/api/guests', { cookie })
      if (cookie) {
        const docs: Record<string, unknown>[] = JSON.parse(guests.text).docs ?? []
        check(
          docs.length === 1 && docs.every((d) => !('code' in d)),
          'sees only their own guest record, without any invitation code',
        )
      } else {
      ...
      for (const path of [
        '/admin',
        '/admin/globals/event',
        '/admin/collections/guests',
        '/admin/collections/guests/1',
      ]) {
        const page = await call('GET', path, { cookie })
        check(
          !page.text.includes(secret) && !/INVITATIONS|Tagged for quiz/i.test(page.text),
          `${path} shows no dashboard data`,
        )
      }
    ...
    check(
      (await call('GET', '/api/quiz-questions', { cookie: couple })).status === 200,
      'GET /api/quiz-questions works',
    )
