...
import { FAKE_MESSAGES } from '../fixtures.js';
...
function Composer({ onSent }) {
  ...
  const submit = async (e) => {
    ...
    setState('sending');
    try {
      const res = await api.post('/api/messages', { body, private: priv });
      onSent(res.message);
      setBody('');
      setState('sent');
...
export default function Messages() {
  ...
  useEffect(() => {
    if (locked) return;
    api.get('/api/messages').then((r) => setMessages(r.messages)).catch((e) => toast(e.message, 'error'));
      ...
      <Gate section="messages">
        ...
        <section className="messages-layout">
          ...
          <div className="message-wall">
            {!locked && messages && messages.length === 0 ? (
