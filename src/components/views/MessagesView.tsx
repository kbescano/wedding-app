...
import { messageView, type MessageView } from '../../lib/views'
import { FAKE_MESSAGES } from '../../lib/fixtures'
...
function MessageCard({
  ...
  m: MessageView
  index: number
  onDelete: ((m: MessageView) => void) | null
}) {
  return (
    <motion.article
...
function Composer({ onSent }: { onSent: (m: MessageView) => void }) {
  ...
  const submit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    setState('sending')
    try {
      const res = await api.post<{ doc: Message }>('/api/messages?depth=0', { body, private: priv })
  ...
  if (role === 'admin') {
    return (
      <aside className="composer card">
        ...
        <p className="muted">
          Guests write here. You can read everything, including private notes, and remove anything from the wall.
        </p>
      </aside>
    )
  }
  ...
  if (!role) {
    return (
      <aside className="composer card">
        ...
        <p className="muted">
          Messages are signed with your name, so they’re for invited guests. Sign in with your invitation code to leave
          one.
        </p>
  ...
  return (
    ...
    >
      ...
      <textarea
        ...
        rows={6}
        maxLength={MAX}
        placeholder="A wish, a memory, some (gentle) advice…"
        value={body}
        onChange={(e) => setBody(e.target.value)}
      />
      <div className="composer-meta">
        <span className={body.length > MAX - 60 ? 'warn' : ''}>
          {body.length}/{MAX}
        </span>
      </div>
      ...
      <p className="signed">
        Signed as <b>Message by {guest?.name || 'Your name'}</b>
      </p>
...
export default function MessagesView({ messages: initialMessages }: { messages: MessageView[] }) {
      ...
      <Gate section="messages">
        <section className="page-head">
          ...
          <Reveal y={20} delay={0.08}>
            <h1 className="h1">
              Words for the <em>couple</em>
            </h1>
          </Reveal>
          <Reveal y={20} delay={0.16}>
            <p className="lede">Leave a few words they can keep forever. Each message is signed with your name.</p>
          </Reveal>
        </section>
          ...
          <div className="message-wall">
            {!locked && messages.length === 0 ? (
              ...
              <AnimatePresence initial>
                {list.map((m, i) => (
                  <MessageCard key={m.id} m={m} index={i} onDelete={locked ? null : remove} />
                ))}
              </AnimatePresence>
            )}
          </div>
