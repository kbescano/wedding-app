...
import { api, uploadPhoto } from '../../lib/client-api.js';
import { photoView } from '../../lib/views';
import { FAKE_PHOTOS } from '../../lib/fixtures.js';
import { prepareImage, timeAgo } from '../../lib/format.js';
...
function Tile({ photo }) {
  ...
  return <img src={photo.thumb || photo.url} alt={photo.caption || `Photo by ${photo.author}`} loading="lazy" decoding="async" />;
}
...
export default function MemoriesView({ photos: initialPhotos }) {
      ...
      <Gate section="memories">
        <section className="page-head">
          ...
          <Reveal y={20} delay={0.24} className="head-actions">
            ...
            {!locked && <span className="count-chip">{photos.length} {photos.length === 1 ? 'photo' : 'photos'}</span>}
          </Reveal>
        </section>
