...
import { FAKE_TABLES } from '../../lib/fixtures'
import type { SeatingData } from '../../lib/views'

const shortLabel = (label: string) => label.match(/\d+/)?.[0] || label.slice(0, 2).toUpperCase()

function RoundTable({ label, count, active }: { label: string; count: number; active: boolean }) {
  ...
  return (
    <svg viewBox="0 0 120 120" className={`round-table ${active ? 'is-active' : ''}`} aria-hidden>
      ...
      <text x="60" y="68" textAnchor="middle" className="table-num">
        {shortLabel(label)}
      </text>
    </svg>
  )
...
export default function SeatingView({ seating }: { seating: SeatingData }) {
          ...
          ? tables
              .filter(
                (t) =>
                  ...
                  t.guests.some((g) => g.toLowerCase().includes(query.toLowerCase())),
              )
              .map((t) => t.label)
          : [],
      ),
      ...
      <Gate section="seating">
        <section className="page-head">
          ...
          <Reveal y={20} delay={0.08}>
            <h1 className="h1">
              Find your <em>seat</em>
            </h1>
          </Reveal>
          <Reveal y={20} delay={0.16}>
            <p className="lede">Look yourself up, or find out who you’ll be sitting with.</p>
          </Reveal>
        </section>
          ...
          <>
            ...
            <div className="search-wrap">
              ...
              <input
                ...
                placeholder="Search a name or table…"
                aria-label="Search guests or tables"
              />
              {query && (
                <span className="search-count">
                  {matches.size} {matches.size === 1 ? 'table' : 'tables'}
                </span>
              )}
            </div>
            ...
            {!locked && query && matches.size === 0 && (
              <p className="muted center">No one by that name. Try a different spelling.</p>
            )}
          </>
        )}
      </Gate>
